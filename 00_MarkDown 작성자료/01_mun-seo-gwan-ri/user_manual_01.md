---
title: 문서관리 사용자교육자료
description: 문서처리부터 문서관리카드, 문서함 화면, 마스킹과 비전자문서 기능까지 안내하는 문서관리 교육자료입니다.
---
...
이송, 반송, 경유, 회신 처리를 위해서는 아래 조건을 모두 만족해야 합니다. 조건을 충족하지 않으면 해당 기능이 비활성화되거나 처리 지연이 발생할 수 있습니다.
...
결재완료된 행정문서는 삭제 및 수정이 불가능합니다. 따라서 개인정보 유출된 공문서는 직원열람제한, 마스킹 처리 등의 제한조치로 개인정보를 보호하면 됩니다.

문서 내 정보주체 또는 제3자의 이익을 부당하게 침해할 우려가 있을 경우에는 교육청 업무관리 총괄운영자와 상의하여 처리하면 됩니다.

!!! Quote "행정문서 내 개인정보보호 처리 관련 법령"
    - 「개인정보 보호법」 제6조(다른 법률과의 관계) ① 개인정보의 처리 및 보호에 관하여 다른 법률에 특별한 규정이 있는 경우를 제외하고는 이 법에서 정하는 바에 따른다. 
    - 「공공기록물 관리에 관한 법률」 제5조(기록물관리의 원칙)에 따르면 공공기관 및 기록물관리기관의 장은 기록물의 생산부터 활용까지의 모든 과정에 걸쳐 진본성(眞本性), 무결성(無缺性), 신뢰성 및 이용가능성이 보장될 수 있도록 관리하여야 한다.
...
- 과제카드함 (1)에서 단위과제에 생성된 과제카드에 담당자를 지정
    ...
    1. :man_raising_hand: :material-sitemap-outline: 업무관리 → 과제관리 → 과제카드관리 → 과제카드함
...
=== "결재정보 탭"
    문서관리카드(기안기)의 결재정보 탭에서는 문서 관련 정보와 결재 상신을 위한 결재경로 등을 입력할 수 있습니다. 결재정보 탭은 문서정보, 결재경로, 공람, 지식공유, 시행정보로 구성되어 있습니다.

    * 문서정보

        문서정보에서는 문서의 기본적인 메타데이터를 입력할 수 있습니다. 제목, 과제카드, 대국민공개여부, 공개제한근거, 공개제한기간, 직원열람범위, 직원열람제한은 필수 입력 항목이며, 제목에는 개인정보가 포함되지 않도록 주의해야 합니다.
        ...
        !!! Info "문서정보 내 입력, 선택 항목"
            ...
            - **대국민공개여부(필수)** : 공개, 부분공개, 비공개 중 선택 (1) 
                ...
                1.	- 정보공개포털 목록(공개)은 대국민공개여부를 비공개로 설정한 경우에 선택 가능
            ...
            - **공개제한근거(필수)** : 대국민공개여부를 부분공개, 비공개 선택 시 제한 근거 법령을 1호 ~ 8호 중 복수 선택 (1) 
                ...
                1.	- 공개제한근거를 5호로 지정 시 공개제한사유를 **감사·입찰계약·인사관리 등**으로 선택하면 공개제한기간이 영구로 지정되며 변경 불가
                    - 공개제한사유를 **내부검토/의사결정** 선택 시 공개제한기간을 지정날짜까지로 선택할 수 있으나 영구로는 선택이 불가
            - 공개제한사유
            - **공개제한기간(필수)** : 공개제한근거가 5호인 경우에 지정날짜까지, 영구 중 선택
            - **직원열람범위(필수)** : 기관(기관 내 열람 가능), 부서(부서 내 열람 가능) 중 선택
            - **직원열람제한(필수)** : 설정안함 (1) , 결재완료일까지 (2) , 지정날짜까지 (3) , 영구 (4)  중 선택
                { .annotate }
                ...
                4.	:man_raising_hand: 직원열람제한에서 영구 선택 시 결재경로상의 사용자 이외의 일반 사용자는 영구적으로 열람 불가
    
            - 기타보안 : 열람시암호확인 (1) , 문서암호화(DRM) (2) 중 복수 선택
                { .annotate }
    
                1.  :man_raising_hand: 기안자는 중요문서 작성 시 문서관리카드 내 `열람시암호확인`을 선택하여, 문서 열람 및 결재/일괄결재 시 인증서를 통한 인증이 이루어지도록 설정
                2.	- 디지털 권리 관리(Digital rights management)의 약자로, 업무관리시스템에서는 내부문서를 외부로 무단 배포하는 것을 방지하기 위하여 사용
                    - DRM 항목을 선택하여 기안한 문서의 본문과 첨부파일을 사용자가 업무관리시스템에 로그인할 수 없는 외부 사용자에게 배포하더라도 해당 파일을 열어볼 수 없도록 암호화하여 저장하는 기능
                    -  각 사용자는 DRM 기능을 사용하기 위해서 PC에 DRM 설치 필수
            
        !!! Info "과제카드 미입력 시 자동 팝업"
            - 과제카드를 입력하지 않은 상태로 문서를 처리하면 필수값 입력 안내 후 과제카드선택 팝업이 자동으로 표시되어 바로 지정 가능
            - 자동 팝업은 **과제카드** 항목에만 적용
            - 자동 팝업이 표시되는 시점
                - 기안 시 : **결재올림**
                - 접수 시 : **접수**
        ...
        !!! Warning "직원열람제한 관련 제약사항"
            ...
            - 대국민공개여부가 공개인 문서는 직원열람제한에서 설정안함만 선택 가능
            - 문서의 직원열람제한을 설정안함 또는 결재완료일까지에서 지정날짜까지로 변경할 경우, 기존에 지정되어 있던 공람자는 자동 해제됨
            - 열람제한문서를 업무인계인수할 때 인계자가 작성한 문서는 인수자에게 자동으로 보안공유 처리되어 열람 가능하며, 인수자가 열람이 불필요한 경우 업무인계인수 시 제외 실적으로 추가해 진행할 수 있음
    
            직원열람제한 시 관련 권한은 아래와 같습니다.
            
            - 열람 권한 : 총괄관리자, 기록물담당자, 결재선, 과제카드 관리자, 감사담당자(감사담당자관리 메뉴에 등록된 사용자), 감사담당자(문서관리 감사담당자 권한), 공람자, 보안공유자, 부서장/학교장
            - 보안공유 지정 권한 : 총괄관리자, 기관관리자(업무), 시도기록물담당자, 기관기록물담당자, 부서(학교)기록물담당자, 감사열람조직, 과제카드 관리자(왕관), 결재선
        ...
        ??? Info "문서암호화(DRM) 문서 관련 안내사항"
            ...
            - DRM 기능은 일부 시도교육청에만 사용 중. 2025년 7월 기준으로 **부산, 대구, 인천, 세종, 강원, 충북, 충남, 전북, 전남교육청**이 DRM 사용이 가능하도록 총괄운영자가 설정
    ...
    * 시행정보
        ...
        ??? Info "자동발송 관련 안내"
            ...
            - 자동발송은 단건 결재 시 사용 가능하며, 일괄기안·일괄결재 시에는 사용할 수 없음
...
=== "본문입력 탭"
    ...
    !!! Info "본문 입력 관련 안내사항"
        ...
        - 내부문서의 본문 및 첨부파일 총 용량 제한은 교육청마다 상이함
        - 첨부파일의 공개여부 선택은 대국민공개여부가 부분공개인 경우에만 가능하며, 이때 첨부파일은 최소 1개 이상 비공개로 설정해야 함

=== "재정문서 탭"
    재정업무에서 기안 요청 시 문서관리카드 팝업의 본문 옆에 재정문서 탭이 추가됩니다. 재정문서 탭에서는 OZD 파일이 출력됩니다.
...
=== "문서관리카드 기능"
    ...
    - 홍보문구
        ...
        결재문서 상하단의 홍보문구를 선택할 수 있습니다. 홍보문구 버튼을 클릭하여 홍보문구 팝업이 뜨면 상부 홍보문구와 하부 홍보문구를 선택하면 됩니다. 홍보문구는 시도총괄관리자, 기관관리자 권한을 보유한 사용자가 등록 및 수정할 수 있습니다.
    ...
    - 공동기안
        ...
        ??? Info "공동기안 관련 안내사항"
            ...
            - 공동기안의 경우 주관기관에서 채번된 동일한 문서번호로 문서함에서 확인가능
                ...
                - 관계기관 처리 후 확인가능 문서함 : 결재한문서
            - 공동기관의 범위 : 동일 교육청 이내(교육청 내, 지원청 내, 학교 등) 관계기관 선택 가능
            - 공동기안은 내부결재만 가능하며 외부발송 필요 시 별도 기안의 첨부로 처리

    - 안추가

        한 문서관리카드에서 여러 개의 안을 추가하여 일괄 기안 시 안추가 버튼을 사용하면 됩니다. 안추가는 **최대 25개**까지 가능합니다. 처음 기안문 추가 시 1안과 동일한 서식으로 추가되고 3안 이상부터 복사할 기안문을 선택할 수 있습니다. 기안문 추가 후에는 안삭제 버튼이 출력됩니다. 기안문 추가 시 자동발송이 불가능합니다. 일괄기안은 안 추가를 통해 일괄로 문서를 기안/결재 할 수 있습니다.
...
!!! Info "공용서식 관련 안내사항"
    ...
    - 간이서식은 내부결재문서로만 사용 가능하며, 서식 분류가 일반서식인 경우에만 외부 발송 가능. 필드를 임의로 삭제해 서식을 만들면 발송되지 않음
...
!!! Info "재정기안 관련 안내사항"
    ...
    - 재정기안의 경우 별도로 필수결재선을 지정할 수 있으며 필수결재선이 설정된 경우 해당하는 결재자를 결재선에 포함하여야 문서를 처리 가능
...
자료집계/연계기안, 나이스연계기안, 교데통연계기안 함으로 구성되어 있고, 자료집계, 국민신문고, 4세대 지능형 나이스, 교육행정데이터통합관리(교데통) 등 시스템을 통해 연계 처리된 문서를 관리할 수 있습니다. 문서를 반송할 수 있고 연계파일 버튼을 클릭하여 해당 문서의 XML파일을 확인할 수 있습니다. 문서상태가 기안대기중인 문서만 반송처리 가능합니다.

!!! Info "나이스연계기안 조회 대상"
    나이스연계기안에서는 기안대기중, 결재중, 기안회수 상태의 문서를 조회 가능. K-에듀파인에서 결재가 완료된 문서와 회수 후 삭제/연계반송 처리한 문서는 나이스로 연계되어 처리되므로 나이스연계기안에서 조회되지 않음

!!! Warning "나이스 연계 데이터 반영"
    나이스와 K-에듀파인 간 연계 데이터는 실시간으로 반영되나, 유통모듈 장애가 발생하면 처리 결과가 반영되지 않을 수 있음. K-에듀파인에서 처리한 문서의 상태가 나이스에 반영되지 않으면 사용자지원서비스로 문의
...
!!! Info "문서종류 색상 구분"
    목록의 문서종류(일반, 수신, 발신) 항목은 해당 문서의 시행종류에 따라 색상이 구분되어 표시됨.
    ...
    시행종류는 문서 선택 후 오른쪽 상단의 문서정보 버튼 또는 목록의 상태 아이콘 클릭 시 목록 하단에 나타나는 문서정보 탭에서 확인 가능

!!! Info "문서종류별 처리구분"
    문서처리 시 선택할 수 있는 처리구분 유형은 문서종류에 따라 다름.

    - **생산문서** : 검토자, 결재자 모두 결재, 반려, 보류, 중단 선택 가능
    - **수신문서** : 반려, 보류, 중단 선택 불가, 문서처리만 가능

    따라서 결재대기 문서를 반려하는 방법도 문서종류에 따라 다름.

    - **생산문서** : 문서처리 시 처리구분을 **반려**로 선택하여 처리
    - **접수문서** : 처리구분에 반려가 없으므로 문서관리카드의 **재지정요청**을 통해 반려

!!! Info "결재처리 시 과제카드 지정"
    - **생산문서** : 기안자가 결재처리할 때 과제카드 지정
    - **접수문서** : 업무담당자가 결재처리할 때 과제카드 지정

    결재처리 시 문서관리카드에서 과제카드 돋보기 버튼이 보이지 않는 경우는 해당 접수문서의 업무담당자로 지정되어 있지 않은 상태임.

!!! Tip "수신문서 일괄처리 기능"
    수신문서를 여러 건 처리할 때 결재대기 화면에서 아래 기능으로 일괄 처리 가능.

    - **일괄과제결재** : 수신문서의 업무담당자로 지정된 경우 과제카드 지정과 문서처리를 일괄 진행
    - **일괄담당자과제지정** : 수신문서의 업무관리자(마지막결재자)로 지정된 경우 업무담당자를 일괄 지정. 본인을 업무담당자로 지정하면 과제카드 지정과 문서처리까지 일괄 진행

??? Info "결재진행중 문서의 결재경로 변경"
    결재가 진행 중인 문서도 결재경로를 변경할 수 있으며, 해당 문서에서의 역할에 따라 사용하는 메뉴가 다름.

    - **기안자, 접수자** : 문서진행에서 결재경로변경으로 수정
    - **검토자, 결재자** : 결재대기에서 문서처리 전 결재경로지정으로 수정

    결재대기에서는 문서제목 선택 후 상단의 결재경로 버튼을 클릭하거나, 결재정보 탭의 결재경로 항목에서 결재경로지정 버튼을 클릭하여 변경 가능. 다만 결재대기에서 문서를 처리하는 사용자가 **협조자**인 경우 결재경로 변경 불가
...
!!! Warning "공람 지정 가능 문서"
    문서진행에서 공람은 **수신문서**인 경우에만 지정 가능. 시행종류가 대내/수신, 대외/수신이 아닌 문서를 공람 지정하면 [공람지정 할 수 없는 문서입니다] 오류 발생

??? Info "회수가 처리되지 않는 경우"
    회수 시 [회수할 수 없는 문서가 있습니다] 안내가 나타나는 주요 원인.

    - 이미 상위 결재자가 결재를 진행한 경우
    - 문서의 상태가 회수할 수 없는 단계로 변경된 경우
    - 일시적인 오류가 발생한 경우

    문서 목록을 재조회하여 현재 상태를 확인한 후 재처리하고, 동일 현상이 반복되면 사용자지원서비스로 문의
...
공람은 문서의 진행 단계에 따라 지정할 수 있는 화면이 다릅니다.

!!! Info "공람지정이 가능한 화면"
    - **기안, 결재 진행 중인 문서** : 문서관리카드의 **공람** 탭에서 공람자 지정. 문서진행, 접수대기에서는 화면 상단의 공람 버튼으로도 지정 가능
    - **완료된 문서** : 기안한문서, 결재한문서, 접수한문서, 연계기안한문서, 문서등록대장, 부서인수문서 (1) 에서 화면 상단의 공람 버튼으로 지정
        { .annotate }

        1.  :man_raising_hand: 부서인수문서 함은 완료된 문서가 부서인수되어 조회되는 문서함
    - **미완료문서/문서진행 함** : 완료되지 않은 문서가 조회되는 문서함이나, 접수문서는 완료 전이라도 접수 시점에 문서번호가 채번되므로 **접수문서에 한해서만** 공람지정 가능
...
공람완료 함에서는 이미 공람확인 완료된 공람문서를 확인할 수 있으며, 목록에서 원하는 문서의 체크박스를 선택한 후 공람의견등록 버튼을 통해 추가 의견을 작성하거나 삭제 버튼을 통해 해당 문서를 삭제할 수 있습니다. 공람완료된 문서 선택 후 삭제 버튼 클릭 시 본인의 공람완료 함에서만 삭제됩니다.
...
이미 지정된 공람을 되돌리는 방법은 **공람회수**와 **공람삭제** 두 가지이며, 문서를 선택하여 열었을 때 화면 하단에 표시되는 **공람정보** 탭에서 처리합니다. 공람정보 탭은 공람대기, 공람완료, 공람지정, 문서등록대장 화면에 공통으로 제공되며 사용자가 보유한 권한에 따라 사용할 수 있는 기능이 다릅니다.

!!! Info "공람회수와 공람삭제 비교"

    - **공람회수**: 사용자에게 공람된 문서는 공람지정자가 공람회수할 수 있으나, 공람자가 공람확인을 완료한 문서는 공람회수 불가. 공람회수 시 공람자의 공람대기 함에서 삭제됨
    - **공람삭제**: 사용자가 공람확인을 완료한 문서도 공람삭제 가능하나, **시도총괄관리자** 또는 **지역총괄관리자** 권한을 가진 사용자만 처리할 수 있으며, 공람삭제 시 공람지정자의 공람지정 함과 공람자의 공람완료 함에서 공람 내역이 삭제됨

    | 구분      | 공람회수                    | 공람삭제                                     |
    | --------- | ------------------------ | ----------------------------------------- |
    | 사용 권한 | 공람지정자                    | 시도총괄관리자(업무), 지역총괄관리자(업무)           |
    | 대상 문서 | 공람확인 전 문서만 가능          | 공람확인 완료 문서도 가능                        |
    | 처리 결과 | 공람자의 공람대기 함에서 삭제      | 공람지정자의 공람지정 함, 공람자의 공람완료 함에서 삭제 |

#### 공람이력

공람이력 함에서는 공람한 문서의 열람이력을 조회할 수 있습니다.
...
!!! Info "발송처리 관련 안내사항"
    - 자동발송으로 설정된 문서는 모바일에서 결재 불가. 자동발송이 필요한 문서는 PC에서 결재 진행
...
같은 수신처로 다시 보내기 위해서는 재발송 버튼을 클릭하면 되고 추가 수신처로 문서를 발송하기 위해서는 추가재발송요청 버튼을 클릭하고 추가재발송요청 팝업에서 수신자, 승인자, 의견작성을 입력하고 확인 버튼을 클릭하면 됩니다.

추가재발송요청의 경우 승인자가 승인 및 취소를 할 수 있으며 승인할 경우 자동으로 발송됩니다. 또한 재발송/추가재발송 된 문서는 취소할 수 없습니다.

!!! Info "재발송, 추가재발송 처리 권한 및 제약"
    - 추가재발송을 처리할 수 있는 사용자 : 해당 문서의 **기안자**, **처리과문서담당자**
    - 처리과문서담당자가 추가재발송요청한 문서는 기안자가 문서등록대장에서 회수 불가
    - 기안자 본인이 추가재발송요청한 문서가 회수되지 않는 경우 화면을 새로고침하여 요청이 이미 승인되었는지 확인. 요청 상태인데도 회수되지 않으면 사용자지원서비스로 문의
    - 재발송은 수신처에서 해당 문서가 완료되지 않은 상태에서는 중복처리되어 발송 불가. 수신문서의 처리 상태 확인 후 재처리
...
!!! Info "배부 관련 안내사항"
    ...
    - 일괄 처리 후 목록에서 문서가 보이지 않는 경우는 삭제된 것이 아니라 다음 단계의 문서함으로 이동한 상태이므로 아래 위치에서 확인 후 처리
        - 접수대기에서 **일괄재배부요청** 처리한 문서 : 배부대기에서 확인 후 배부 처리
        - 배부대기에서 **일괄배부** 처리한 문서 : 접수대기에서 확인 후 접수 처리


!!! Info "접수 관련 안내사항"
    - 접수 상태 문서의 결재경로 변경은 접수대기에서만 가능
    - 재발송요청 상태의 문서는 접수 처리할 수 없으며 추가기능의 재발송요청문서삭제 버튼을 통해 해당 문서 삭제
    - 유통문서의 반려는 접수대기에서 재발송요청으로 처리. 접수처리한 문서는 문서번호가 채번되어 재발송요청 불가
    - 수신도착, 대외도착 상태의 문서는 저장 기능을 지원하지 않으므로 접수처리 후 저장
    - 접수대기 목록에 기안자 항목이 제공되어 문서를 열람하지 않아도 기안정보 확인 가능
    - 접수대기 목록의 엑셀다운로드 시에도 기안자 항목 포함
    - 접수대기 진입 경로에 따라 처리상태 조회조건의 기본값이 다름
        - 알림존을 통해 이동한 경우 : **도착**, **접수**, **재지정요청** 선택
        - 접수대기 메뉴로 직접 진입한 경우 : **도착**, **접수**, **진행**, **재지정요청**, **재발송요청** 선택
...
담당확인요청은 접수대기에서 문서를 접수하기 전에 해당 문서의 담당자가 맞는지 확인받기 위한 기능으로, 결재경로를 지정하기 전 단계에서 활용합니다. 요청을 받은 사용자는 업무담당확인 함에서 해당 문서를 선택한 후 담당확인 버튼을 클릭하여, 처리구분을 담당확인 또는 재확인요청 중에서 선택해 처리합니다.

!!! Info "재확인요청과 재지정요청의 차이"
    - **재확인요청** : 업무담당확인 함에서 담당확인요청 받은 문서를 처리할 때 선택하는 처리구분 중 하나
    - **재지정요청** : 업무관리자, 업무담당자의 재지정을 요청하는 별도 기능이며 요청된 문서는 재지정요청 함에서 확인

#### 과제미지정

과제미지정 함에서는 처리과문서담당자가 과제카드를 지정하지 않은 문서를 확인할 수 있으며, 해당 문서는 미완료문서 함에서 과제카드를 지정하여 처리할 수 있습니다.

#### 재지정요청

재지정요청 함에서는 처리과문서담당자가 재지정요청된 문서를 확인할 수 있으며, 해당 문서는 접수대기, 미완료문서, 재지정요청 함에서 담당자를 다시 지정하여 처리할 수 있습니다.

### :material-folder: 내문서

내문서 하위 문서함에서는 본인이 기안, 결재, 접수한 문서와 본인에게 공유된 문서를 확인할 수 있습니다.

!!! Info "내문서함 문서 삭제 관련 안내"
    - 삭제 가능한 문서함 : 기안한문서, 결재한문서, 접수한문서, 연계기안한문서, 부재중문서
    - 목록에서 삭제한 문서는 해당 문서함에서 복구 불가
    - 삭제한 문서는 문서등록대장에서 조회 가능

#### 기안한문서

기안한문서 함에서는 본인이 기안한문서를 확인할 수 있습니다. 문서 선택 후 첨부물분리등록 가능하고 발송문서인 경우 발송요약정보도 확인할 수 있습니다.
...
접수한문서 함에서는 완료된 문서중 본인이 결재한 접수문서를 확인할 수 있습니다. 상단 검색조건의 공문게시여부는 문서등록대장과 동일하게 전체, 공문게시, 공문게시(외부공문제외), 공문게시제외, 외부공문제외로 세분화되어 있어 다양한 조건으로 조회할 수 있습니다.
...
문서등록대장에서는 소속된 조직에서 생산, 접수한 전체 문서를 조회할 수 있으며 권한에 따라 마스킹, 첨부물분리등록, 비전자문서등록, 수정, 공람, 재발송 등의 기능을 사용할 수 있습니다. 

문서등록대장에 등록된 문서는 결재가 완료되었거나 접수되어 문서번호가 채번된 문서이며, 기록물 이관 대상으로 관리되므로 삭제할 수 없습니다. 다만 문서 하단 공람정보 탭의 공람회수와 공람삭제 (1)는 문서 자체가 아닌 공람 내역만 처리하는 기능이므로 문서 삭제 제한과는 무관합니다.
{ .annotate }

1.  :man_raising_hand: 공람회수는 공람지정자가 공람확인 전 건에 한해, 공람삭제는 시도총괄관리자(업무)와 지역총괄관리자(업무)가 공람확인 완료 건까지 처리 가능

문서등록대장의 등록일자를 통한 최대 조회 기간은 3개월~1년 사이로 시도교육청과 학교별 설정이 다릅니다. 긴 기간을 검색할 때는 날짜를 조정해 여러 번 나누어 조회해야 합니다.
...
??? Info "문서등록대장의 검색조건"
    ...
    - 소속부서 (1) 
        ...
        1.  :man_raising_hand: 소속부서에서 사용자가 소속된 조직을 최상위로 하위부서포함 여부를 선택할 수 있으며 조직도 팝업을 통해 열람권한이 있는 특정 조직과 해당 조직의 하위부서를 포함한 문서 조회 가능
            - 하위 부서의 열람제한 문서는 기관장 권한 사용자만 열람 가능
    - 감사열람조직 (1) 
        { .annotate }
    ...
    - 열람제한 : 전체, 열람제한, 열람제한 제외 중 택일 (1) 
        ...
        1.  - 열람제한에서 열람제한이 설정되어 있고 열람제한 지정날짜가 지나지 않은 문서, 열람제한이 영구로 설정된 문서 조회
            - 열람제한이 결재완료일까지로 설정된 문서는 최종 결재완료 시 열람제한 문서에서 제외되므로 해당 검색 옵션에서도 제외됨
    - 문서종류 : 전체, 전자, 비전자
    - 소급여부
    - 키워드

??? Info "문서등록대장 기능 안내"
    - 한건수정 가능 항목 : 과제카드명, 관련정보, 대국민공개여부, 직원열람범위, 직원열람제한
    - 일괄수정 가능항목 : 대국민공개여부, 직원열람범위, 직원열람제한


#### 문서등록대장(다년도검색)

기존 문서등록대장은 한 번에 설정할 수 있는 검색 기간이 최대 1년으로 제한되어 있어, 1년이 넘는 기간을 검색하려면 여러 번 나누어 검색해야 했습니다.

또한 사용자가 집중되는 시기에는 속도도 느려지는 불편함이 있어 이를 개선하고자 문서등록대장(다년도검색) 기능이 개발되었습니다. 다년도검색은 색인 검색 방식으로 검색 속도가 빠르고 검색 기간에 제한이 없다는 장점이 있습니다.

기존 문서등록대장에서 제공하는 검색조건 중 사용빈도가 높은 15개 조건을 다년도검색에서도 동일하게 제공하고 있습니다.

!!! Warning "다년도검색의 제약사항"
    - 매일 0시에 전일 문서를 증분색인하기 때문에 당일 생산된 문서는 조회가 되지 않음
    - 폐지 조직 문서는 문서 인수인계가 되더라도 생산 당시 부서명으로 목록에 출력
    - 검색 결과를 엑셀 파일로 내려받을 때는 최대 1만 개의 목록만 저장되며, 이를 초과하는 경우에는 검색 기간을 나누어 저장해야 함
...
처리자의 부재나 인사이동, 폐교 등의 사유로 장기적으로 미처리되고 있는 문서가 해를 넘겨 처리완료되지 않도록 확인 및 처리가 필요하며, 원활한 문서 정리를 위해 장기간 미접수된 문서를 한 번에 일괄 처리할 수 있는 기능을 제공하고 있습니다.

!!! Info "미완료문서 조회 범위 제한"
    - 미완료문서 함에서 타 부서 문서를 조회하는 **조직검색 버튼**은 2024년 제5차 업무협의체 결정에 따라 제거
    - 교육청 총괄운영자 등이 하위조직의 미접수 문서 현황을 확인하고자 할 경우 조회쿼리를 작성하여 데이터 추출이 필요
...
부서인수문서에서는 과제카드 인계인수를 통해 인계한 문서를 인수부서에서 확인할 수 있습니다. 
...
결재된 문서의 내용은 수정할 수 없기 때문에 문서 원본은 수정되지 않은 채 문서관리카드 밖의 화면에서는 마스킹 처리되어 표기됩니다. 제목, 첨부파일명에 대해서 설정할 수 있으며 마스킹을 원하는 단어에 직접 *을 입력하여 적용합니다.
...
문서과문서담당자 권한의 사용자는 배부대기, 처리과문서담당자 권한의 사용자는 접수대기에서 접수 비전자문서를 등록, 처리할 수 있습니다. **배부대기, 접수대기**에서 등록된 비전자문서는 **대외도착**, **수신도착** 상태일 때 수정 또는 삭제 가능합니다.

문서등록대장에서 생산한 비전자문서는 전자문서로 변경할 수 없습니다. 전자문서가 필요한 경우에는 해당 내용으로 문서를 새로 기안하여 전자문서를 생산해야 합니다.

비전자문서는 본문이 없는 문서이므로 공문게시 대상에서 제외되며, 공문게시가 필요한 경우에는 본문이 있는 일반문서로 기안하여 수신자를 지정한 후 발송해야 합니다.

!!! Info "비전자문서 결재 처리 방법"
    - **접수함 → 접수대기** : [비전자문서] > [비전자문서등록]에서 결재 담당자를 지정하여 접수 진행 시 결재 처리 가능
    - **문서함 → 문서등록대장** : 등록구분에 따라 기안자와 결재자 또는 접수자와 업무담당자 지정 가능하나, 별도 결재 진행 없이 완료 처리되어 등록
    - 문서등록대장에 등록된 비전자문서는 열람 후 상단 [수정] 버튼으로 결재선 변경 가능
...
??? Info "비전자문서 등록 항목"	
    ...
    - **원기안(발신)부서(필수)** : 원기안(발신)부서명을 텍스트로 입력
...
!!! Info "첨부물분리등록 관련 안내사항"
    ...
    - 분리등록 내역이 과제카드의 표지·색인목록에 조회되지 않는 경우는 표지색인출력 화면의 **문서구분** 조회조건이 기본값인 **비전자**로 설정되어 있기 때문. 첨부물분리등록 정보는 전자문서 관련 데이터이므로 문서구분을 **전자** 또는 **전자+비전자**로 변경 후 조회
...
=== "수정"
    ...
    !!! Tip "여러 문서의 대국민공개여부를 일괄수정 방법"
        문서별로 개별 수정할 수도 있지만, 여러 건은 일괄수정 기능을 사용해 업무효율을 높일 수 있습니다.

        1. 변경할 문서들을 목록에서 선택
        2. 메뉴에서 대국민공개여부 일괄수정 실행
    ...
    !!! Info "한건수정, 일괄수정 관련 안내사항"
        ...
        - 일괄수정은 부분공개로 수정 불가
        - 한건수정 가능항목 : 과제카드명, 관련정보, 대국민공개여부, 직원열람범위, 직원열람제한, 쪽수(개수)
        - 일괄수정 가능항목 : 대국민공개여부, 직원열람범위, 직원열람제한
        - 열람제한 문서는 해당 문서의 결재선에 포함된 사용자만 수정 가능
        - 비전자문서의 경우 문서등록대장에서 문서관리카드 상단의 수정 버튼을 통해 수정 가능
        - 한건수정 시 과제카드 변경기간이 아닌 경우 과제카드 돋보기 비활성화
        - RMS로 기록물이관된 문서의 경우 한건수정, 일괄수정 불가

    !!! Info "수정 시 개인정보필터 사용"

        대국민공개여부를 비공개에서 공개로 전환하는 경우 수정 팝업에서 개인정보 포함 여부를 확인할 수 있습니다.

        - 시도교육청 문서환경설정 (1)에 '한건수정, 일괄수정 개인정보필터 사용여부'가 설정된 경우에만 제공
            { .annotate }

            1.  :man_raising_hand: :material-sitemap-outline: 서비스공통 → 포털관리 → 운영환경관리 → 문서환경설정

        - 선택한 문서 중 비공개에서 공개로 전환하는 문서만 검출 대상
...
=== "보안공유"
    ...
    보안공유 버튼은 **문서등록대장**, **기안한문서**, **부서인수문서**에서 사용할 수 있습니다.

    !!! Warning "보안공유 지정 권한 축소 예정"
        2026년 제2차 업무협의체 결정에 따라 보안공유 지정 권한은 **과제카드 관리자**, **결재선**으로 축소 예정
    ...
    !!! Tip "보안공유자 등록 방법"
        ...
        보안공유 및 열람제한 열람 권한은 다음과 같습니다.

        - 보안공유 지정 : 총괄관리자, 기관관리자(업무), 시도기록물담당자, 기관기록물담당자, 부서(학교)기록물담당자, 감사열람조직, 과제카드 관리자(왕관), 결재선
...
=== "공람, 사후보고"
    ...
    !!! Info "공람, 사후보고"
        ...
        - 공람자 지정 시 결재선과 동일하게 직위가 표시되어 동명이인 구분 가능
        - 지정된 공람은 문서 하단 공람정보 탭에서 권한에 따라 공람회수 (1) 또는 공람삭제 (2) 처리 가능
            { .annotate }

            1.  :man_raising_hand: 공람지정자가 처리하며 공람자가 공람확인을 하지 않은 건만 가능
            2.  :man_raising_hand: 시도총괄관리자(업무), 지역총괄관리자(업무)가 처리하며 공람확인을 완료한 건도 가능


=== "과제변경"

    완료된 문서 중 과제카드 변경이 필요한 경우 과제카드변경기간 (1) 내에선 과제카드를 변경할 수 있습니다. 생산문서의 경우 **기안자**, 수신문서의 경우 **업무담당자와 처리과문서담당자**가 과제카드를 변경할 수 있으며, 사용하는 기능에 따라 처리 가능한 권한과 화면이 추가됩니다.
    { .annotate }
    ...
    !!! Info "기능별 과제카드 변경 권한"
        - **수정 → 한건수정** : 기안자, 업무담당자, 문서과문서담당자, 처리과문서담당자
        - **추가기능 → 과제변경** : 기안자, 업무담당자, 처리과문서담당자, 문서과문서담당자, 과제카드 관리자(왕관)

    ??? Info "한건수정으로 과제카드를 변경할 수 있는 화면"
        - 문서관리 → 내문서함 → 기안한문서
        - 문서관리 → 문서함 → 문서등록대장
        - 문서관리 → 문서함 → 부서인수문서
        - 과제관리 → 나의과제 (과제카드 → 실적관리)
        - 과제관리 → 부서과제 (과제카드 → 실적관리)
        - 과제관리 → 과제카드관리 → 과제카드함 (과제카드 → 실적관리)

        추가기능 → 과제변경은 문서관리 → 문서함 → 문서등록대장에서만 사용 가능

    !!! Tip "과제카드 변경 방법"
        완료된 문서 중 과제카드 변경이 필요한 문서에 대해 과제카드를 변경할 수 있습니다.

        1. 완료된 문서 중 과제카드가 변경이 필요한 문서들을 선택
        2. 메뉴에서 추가기능 → 과제변경을 실행
        3. 과제카드담당자로 등록되어 있는 과제카드 목록에서 변경할 과제카드를 선택하여 확인
...
=== "공문게시"
    ...
    결재가 완료된 문서도 공문게시판에 게시할 수 있으며 문서종류에 따라 사용하는 메뉴가 다릅니다.

    !!! Info "결재완료 문서의 공문게시 방법"
        - **발송문서** : 발송대기에서 해당 문서를 클릭한 후 **공문게시로변경요청**으로 게시
        - **접수문서** : 문서등록대장에서 해당 문서를 클릭한 후 **공문게시지정**으로 게시
        - 접수대기 단계의 문서는 문서관리카드에서 공문게시지정 후 접수처리하여 게시
        - 공문게시는 대국민공개여부가 **공개**인 문서만 가능. 공개가 아닌 문서는 문서등록대장의 한건수정으로 대국민공개여부를 공개로 변경한 후 게시

    !!! Warning "공문게시 대상에서 제외되는 수신처"
        접수대기에서 공문게시를 진행할 때 **조직도**로 지정된 수신처만 공문게시 대상이며, **행안부유통**, **문서24**, **수기입력**으로 지정된 수신처는 제외됨. 이 경우 [접수공문게시일 경우 LDAP 및 수기입력은 제외됩니다] 알림이 표시되며, 나머지 수신처에 대한 공문게시는 정상 진행
...
=== "참조기안"
    ...
    문서 작성 시 참고할 문서를 첨부파일로 추가하여 기안할 수 있는 기능이며 본문에 참고문서의 문서번호, 일자, 제목이 관련 근거 문구로 자동 입력됩니다.
    ...
    !!! Info "본문 자동 입력 문구"
        문서번호와 일자에 더해 참고문서의 **제목**도 함께 입력되며, 참고문서의 종류에 따라 아래와 같이 작성됩니다.

        - **생산문서** : `1. 관련: 문서번호(결재일자), "문서제목"`
        - **접수문서** : `1. 관련: 발신기관문서번호(시행일자), "문서제목"`



## 통계

통계에서는 문서의 생산, 발송, 접수 현황과 정보공개 현황을 다양한 기준으로 집계하여 조회할 수 있습니다.

기관의 원문공개율은 **기관별정보공개현황**에서 확인하며 이 메뉴는 **정보공개담당자** 권한으로만 조회할 수 있습니다. 그 외 통계 메뉴는 **시도통계담당자**, **지원청통계담당자**, **기관통계담당자**, **부서통계담당자** 권한으로 조회합니다.
...
:   문서관리의 통계 하위 기관별정보공개현황 메뉴에 접근 및 기관 원문공개율 조회
