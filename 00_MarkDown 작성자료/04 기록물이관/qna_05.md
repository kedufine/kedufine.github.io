...
!!! question "전자기록물 이관 전에 시스템 오류 방지를 위해 일괄 보정을 요청하고 싶은데, 언제 해야 하나요?"
    전자기록물 이관 전에는 시스템 오류를 막기 위해 최소 2~3주 전에 사전 환경설정 확인 및 보정을 요청해야 합니다

!!! question "기록물 이관 중 오류가 발생하여 다시 처음부터 진행하려면 어떻게 해야 하나요?"
    기록물 이관 중 오류가 발생했을 경우, 이관실행현황에서 [이관초기화]를 진행한 뒤 기록물정리현황에서 ‘기록물정리완료취소 → 과제카드수집 → 기록물정리완료 → 이관요청’을 다시 수행해야 처음부터 재이관할 수 있습니다.
...
!!! question "K-에듀파인에서 이관 완료된 기록물의 수량이 RMS와 일치하지 않는 경우 어떻게 해야 하나요?"
    이관 완료 후 K-에듀파인과 RMS의 기록물 수량이 불일치하면, 기록물이관수량검증 메뉴에서 처리과별·과제카드별 비교를 진행하고, 차이가 확인되면 해당 자료를 첨부해 사용자지원서비스에 등록해야 합니다.
