...
description: 개인정보관리, 수신그룹, 결재선, 문서환경설정, 알림, 문서의견조회까지 다루는 개인환경설정 교육자료입니다.
---
...
- :material-sitemap-outline: 개인설정 메뉴


## 개인정보관리

개인정보관리에서는 시스템에서 사용하는 사용자의 기초정보를 설정할 수 있습니다. 공문서 하단 표시 항목은 실제 문서 하단에 기입되는 내용이므로 정확하게 입력해야 합니다. 또한 사용자의 겸임 또는 자료권한이 부여된 조직별로 개인정보를 각각 설정할 수 있습니다.

!!! Info "개인정보관리에서 설정 가능한 항목 안내"

    - 일반항목 : 부서명, 사용자ID, 사용자명, 직위, 부서장여부, 개인번호, 대리결재자, 직급, 담당업무, 이동전화번호, 업무사용자 (1) 
        { .annotate }
        
        1.  :man_raising_hand: 업무사용자는 해당 사용자의 시스템 내 업무 수행 가능 여부를 나타내는 항목으로, 휴직·파견 등 장기 부재 시 **아니요**로 설정. **아니요**로 설정 시 결재경로 지정 또는 모든 업무화면에서 조회 불가능
...
개인수신그룹관리에서는 각 사용자별로 개인화된 수신그룹을 등록하고 관리할 수 있습니다. 수신유형(문서, 메모, 일정, 메일, 공람)별로 구분하여 등록해야 합니다.

개인수신그룹의 반영 여부가 **반영**이고 공개 여부가 **공개**로 설정된 경우, 같은 조직 내 다른 사용자가 해당 수신그룹을 조회하고 등록할 수 있습니다.

공람 수신그룹에 구성원을 등록할 때에는 결재선과 동일하게 직위가 함께 표시되므로 동명이인도 혼선 없이 지정할 수 있습니다. 직위 표시는 시도교육청에서 관리하는 수신그룹관리 (1) 화면과 문서관리카드의 공람 등록, 문서함의 공람지정 팝업에도 동일하게 적용됩니다.
{ .annotate }

1.  :man_raising_hand: :material-sitemap-outline: 서비스공통 → 포털관리 → 운영환경관리 → 수신그룹관리
...
!!! Info "개인수신그룹관리 기능 세부안내"
    ...
    - 순서조정 : 조회된 개인수신그룹 목록의 순서 조정
    - 신규 : 문서, 메모, 일정, 메일, 공람 기능별 수신그룹 등록
    - 일괄등록 : 제공 중인 파일 양식을 통해 엑셀파일 업로드하여 수신그룹 일괄등록 가능
    - 일괄수정 : 미반영 상태의 개인수신그룹 이름·구성원 등 일괄 수정
    - 삭제 : 미반영 개인수신그룹 일괄 삭제 
    - 반영 : 미반영 상태의 개인수신그룹을 **반영 상태**로 변경하여 실제 수신 시 사용 가능하도록 활성화
    - 반영취소 : 반영 상태의 개인수신그룹을 **미반영 상태**로 변경하여 수신 시 사용 목록에서 제외


## 나의결재선관리

나의결재선관리에서는 문서 처리 시 사용할 결재선 그룹을 등록하고 관리할 수 있습니다. 결재선 구분은 기안, 접수, 비전자문서(문서등록대장)로 나뉘며, 각 그룹별로 여러 건의 결재선을 추가하고 수정 (1) 할 수 있습니다.
{ .annotate }

1.  :man_raising_hand: 수정된 결재선은 맨 하단으로 순서 변경

결재선 구분과 화면의 대응은 다음과 같습니다. 기안 화면에서는 **기안** 그룹, 접수 처리 시에는 **접수** 그룹, 문서등록대장의 비전자문서 처리 시에는 **비전자문서** 그룹의 결재선이 노출됩니다. 결재선 지정 팝업에서 나의결재선 탭을 선택하고 원하는 그룹을 클릭하면 등록해 놓은 사용자가 결재선에 자동으로 지정됩니다.
...
!!! Info "개인문서환경설정에서 설정 가능한 항목 안내"
    ...
    - 문서관리카드 UI유형 : 보통 (1), 크게 (2), 가로 (3)
        ...
        2.  :man_raising_hand: 문서관리카드 UI 크게
            - 결재 업무가 잦은 사용자를 위한 유형으로, 결재정보와 본문탭이 분리되어 가로로 나열
            - 상단 버튼 크기를 키우고 화면 좌측으로 배치하여 조작성을 높임
            - 결재정보 영역에 접기·펼치기 기능 제공으로 스크롤 없이 주요 정보 한눈에 확인 가능
            - 문서관리카드 내 폰트 크기 19로 제공
        3.  :man_raising_hand: 문서관리카드 UI 가로
            - 결재정보와 본문탭을 한 화면에서 동시에 확인할 수 있는 유형
            - 가로형·고해상도 모니터 환경에서 탭을 오가지 않고 한 번에 내용을 검토할 때 유용
